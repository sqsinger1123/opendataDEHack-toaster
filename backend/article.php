...
if(strlen($row['image'])) {
	$row['text_colspan']		= 1; // colspan of the td that will contain the text.
...
<table cellpadding="0" cellspacing="0" style="width: 600px; table-layout:fixed; border-collapse: collapse; border-spacing:0;">
	...
	<tr>
		<td colspan="2" valign="top" width="580px" style="width: <?= $row['text_width']; ?>px; padding:5px 10px; border:0;">
			<? if(strlen($row['link'])) { ?> <a href="<?= $row['link']; ?>" style="color:#C00 !important; text-decoration:none;"> <? } ?>
	...
	<tr>
		...
		<? if(strlen($row['image'])) { ?>
			<td colspan="1" valign="top" class="w150" width="150px" style="width:150px; padding:5px 0; border:0;">
